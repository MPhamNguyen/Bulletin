    ...
    override suspend fun getCurrentSeller(): Result<ListingSeller> {
        return when (val result = authRepository.getCurrentUser()) {
            is Result.Success -> {
                ...
                if (profile == null) {
                    Result.Error(IllegalStateException(CreateListingErrorMessages.AUTHENTICATION_REQUIRED))
                } else {
                    Result.Success(ListingSeller(SellerId(profile.id.value), profile.fullName))
                }
            }
            is Result.Error -> Result.Error(
                IllegalStateException(CreateListingErrorMessages.AUTHENTICATION_REQUIRED, result.exception)
            )
        }
    }
}
